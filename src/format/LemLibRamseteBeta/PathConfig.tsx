...
export class PathConfigImpl implements PathConfig {
  ...
  @IsBoolean()
  @Expose()
  ramseteBackwards: boolean = false;
  ...
  return (
    <>
      <PanelBox marginTop="0px" marginBottom="16px">
        <FormCheckbox
          label="Reverse path"
          title="Export and simulate this whole Ramsete path backwards"
          checked={pc.ramseteBackwards}
          onCheckedChange={value =>
            app.history.execute(
              `Change path ${pc.path.uid} Ramsete reverse`,
              new UpdateProperties(pc, { ramseteBackwards: value })
            )
          }
        />
      </PanelBox>
